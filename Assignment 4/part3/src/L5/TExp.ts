...
*/
import R, { chain, comparator, concat, map, uniq } from "ramda";
...
// UnionTExp
export type UnionTExp = { tag: "UnionTExp"; comp: TExp[]}
export const makeUnionTExp = (comp: TExp[]): UnionTExp =>
  ({ tag: "UnionTExp", comp: comp });
...
const parseUnion = (texps: Sexp[]): Result<UnionTExp> => {
    if(texps.length !== 2)
        return makeFailure("expected 2 got more or less")
    if(texps[0] === "union")
        return makeFailure("got union incorrect")
    
    const check = add([], texps, 0, texps.length)
    if(!isOk(check))
        return check
    const arr = check.value
    arr.sort(sort_func)
    return makeOk(makeUnionTExp(arr))
}

const add = (arr: TExp[], texps: Sexp[], j: number, length: number): Result<TExp[]> => {
    if(j === length)
        return makeOk(arr)
    const TExp_parse = parseTExp(texps[j])
    if(isFailure(TExp_parse))
        return TExp_parse
    if(isOk(TExp_parse)){
        const texp = TExp_parse.value
        const tag = TExp_parse.tag
        if(texp.tag === "TVar")
            return makeFailure("Illegal argument")
        if(texp.tag === "UnionTExp"){
            const comp = texp.comp
            comp.map((c) => {
                if(!Contain(arr, c.tag)) 
                    arr.push(c)})
        }else{
            if(!Contain(arr, texp.tag)){
                arr.push(texp)
            }
        }
    }
    return add(arr, texps, j + 1, length)
}

export const Contain = (arr: TExp[], texp: string): boolean => {
    return arr_contain(arr, texp, 0, arr.length)
}

const arr_contain = (arr: TExp[], texp: string, i: number, length: number): boolean => {
    if(i === length)
        return false
    if(arr[i].tag === texp)
        return true
    return arr_contain(arr, texp, i + 1, length)
}
...
const get_Tag = (texp: TExp): string => {
    if(isAtomicTExp(texp)) 
        return texp.tag
    else if(isCompoundTExp(texp)) 
        return texp.tag
    else if(isTVar(texp))
        return texp.tag
    else
        throw new Error('Wrong TExp')
}
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isProcTExp(x) ? bind(unparseTuple(x.paramTEs), (paramTEs: string[]) =>
                            mapv(unparseTExp(x.returnTE), (returnTE: string) =>
                                [...paramTEs, '->', returnTE])) :
        isUnionTExp(x) ? makeOk(`(union ${unparseUnion(x.comp, "", 0)})`) :
        isEmptyTupleTExp(x) ? makeOk("Empty") :
        isNonEmptyTupleTExp(x) ? unparseTuple(x.TEs) :
        x === undefined ? makeFailure("Undefined TVar") :
        x;
        const unparseUnion = (comp: TExp[], ret: string, i: number): string => {
            const s1 = up(comp[i])
            const s2 = up(comp[i + 1])
            if(isOk(s1)){
                const val1 = s1.value
                if(isOk(s2)){
                    const val2 = s2.value
                    ...
                    return unparseUnion(comp, ret + val1 + " (union ", i + 1) + ")"
                }
            }
            return ""
        }
