...
import { Result, makeOk, makeFailure, bind, mapResult, mapv, safe2, isOk, isFailure } from "../shared/result";
...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | CondExp | CClause | EClause;
...
export type EClause = {tag: "EClause", then: CExp}
export type CClause = { tag: "CClause", test: CExp, then: CExp[]; }
export type CondExp = { tag: "CondExp", cclauses: CClause[], eClause: EClause}
...
export const makeEClause = (then: CExp) : EClause =>
    ({ tag: "EClause", then: then})
export const makeCClause = (test: CExp, then: CExp[]) : CClause =>
    ({ tag: "CClause", test: test, then: then })
export const makeCondExp = (cclauses: CClause[], eClause: EClause) : CondExp =>
    ({ tag: "CondExp", cclauses: cclauses, eClause: eClause })
...
export const isEClause = (x: any): x is EClause => x.tag === "EClause"
export const isCClause = (x: any): x is CClause => x.tag === "CClause"
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isCondExp(x) || isCClause(x) || isEClause(x);
...
// parseCondExp -> (cond then<exp>+ else<exp>+)
export const parseCondExp = (params: Sexp[]): Result<CondExp> =>{
    if(params.length < 2) 
        return makeFailure(`Expression not of the form (cond <cexp>+ <cexp>+): ${JSON.stringify(params, null, 2)}`) 
    if(!allT(isCompoundSexp, params)) 
        return makeFailure("Invalid cond clauses")
    else{
        const clausesResult = mapResult((clause: Sexp[]) =>
            safe2((test: CExp, then: CExp[]) => makeOk(makeCClause(test, then)))
                (parseL31CExp(first(clause)), mapResult(parseL31CExp, rest(clause))), params.slice(0, -1));
        const toCheck = params[params.length-1][0]
        if(toCheck !== "else"){
            return makeFailure("missing else clause");
        }
        const CElse = parseL31CExp(params[params.length-1].slice(1))
        if(isOk(CElse)){
            const okElse = CElse.value
            return bind(clausesResult, (clauses: CClause[]) => makeOk(makeCondExp(clauses, makeEClause(okElse))));
        }
        else
            return makeFailure("Invalid else caluse")
    }
}
...
const isSpecialForm = (x: string): boolean =>
    ["cond", "if", "lambda", "let", "quote"].includes(x);
...
const unparseCondExp = (cl : CClause[]) : string =>
    map((ccls: CClause) => unparseL31(ccls), cl).join(" ")
...
export const unparseL31 = (exp: Program | Exp): string =>
    ...
    isCondExp(exp) ? `(cond ${unparseCondExp(exp.cclauses)} ${unparseL31(exp.eClause)})` :
    isCClause(exp) ? `(${unparseL31(exp.test)} ${map((ce: CExp) => unparseL31(ce) , exp.then)})` :
    isEClause(exp) ? `(else ${unparseL31(exp.then).slice(1, -2)})` :
    exp;
